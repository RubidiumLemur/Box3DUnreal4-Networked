...
public:
	...
	/**
	 * Applies a server-authoritative radial linear impulse to dynamic bodies inside Radius.
	 * The Box3D broadphase gathers candidates, so bodies outside the blast are not scanned.
	 * This path is intended for high-count instanced actors.
	 * Strength is expressed in the same Unreal-space impulse units as UBox3DBodyComponent::AddImpulse.
	 * Returns the number of unique bodies affected. Call this on the authority/server; clients
	 * reject the operation and receive the resulting motion through normal replication.
	 */
	int32 ApplyRadialImpulse(const FVector& Center, float Radius, float Strength,
		float FalloffExponent, bool bWakeBodies, const FBox3DQueryFilter& Filter,
		float UpwardStrength = 0.0f);

	/** Blueprint radial impulse using the default Box3D collision filter. */
	UFUNCTION(BlueprintCallable, Category = "Box3D|Physics", meta = (DisplayName = "Apply Radial Impulse", ToolTip = "Applies a server-authoritative radial impulse using the Box3D spatial cache. Intended for high-count instanced actors."))
	int32 ApplyRadialImpulseSimple(const FVector& Center, float Radius, float Strength,
