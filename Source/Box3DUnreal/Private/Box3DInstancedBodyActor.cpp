...
namespace
{
	FTransform MakeSafeInstanceTransform(const FTransform& Input)
	{
		FTransform Result = Input;
		FQuat Rotation = Result.GetRotation();
		Rotation.Normalize();
...
void ABox3DInstancedBodyActor::Tick(float DeltaSeconds)
{
	...
	for (int32 Index = 0; Index < ClientCurrentTransforms.Num(); ++Index)
	{
		...
		Current = MakeSafeInstanceTransform(Current);
...
int32 ABox3DInstancedBodyActor::SpawnBody(const FTransform& WorldTransform)
{
	...
	const int32 MaxInstances = GetDefault<UBox3DProjectSettings>()->MaxInstancesPerActor;
	if (MaxInstances > 0 && Bodies.Num() >= MaxInstances)
	{
		ABox3DInstancedBodyActor* OverflowActor = GetOrCreateOverflowActor();
		if (OverflowActor == nullptr)
		{
			return INDEX_NONE;
		}

		const int32 LocalBodyId = OverflowActor->SpawnBody(WorldTransform);
		if (LocalBodyId == INDEX_NONE)
		{
			return INDEX_NONE;
		}

		const int32 GlobalBodyId = NextBodyId++;
		FBodyRoute& Route = RoutedBodies.Add(GlobalBodyId);
		Route.Actor = OverflowActor;
		Route.LocalBodyId = LocalBodyId;
		return GlobalBodyId;
	}

	return SpawnBodyInThisActor(WorldTransform);
}

int32 ABox3DInstancedBodyActor::SpawnBodyInThisActor(const FTransform& WorldTransform)
{

	const FTransform SafeWorldTransform = MakeSafeInstanceTransform(WorldTransform);
	b3BodyDef Def = b3DefaultBodyDef();
	Def.type = b3_dynamicBody;
	Def.position = Box3D::ToBox3DPosition(SafeWorldTransform.GetLocation());
	Def.rotation = Box3D::ToBox3DQuat(SafeWorldTransform.GetRotation());
	...
	Instance.Scale = SafeWorldTransform.GetScale3D();
	Instance.InstanceIndex = InstanceComponent->AddInstanceWorldSpace(SafeWorldTransform);

	FBox3DInstancedBodyState& State = PendingSnapshot.AddDefaulted_GetRef();
	State.Id = Instance.Id;
	State.WorldTransform = SafeWorldTransform;
	NextChunkToReplicate = 0;
	return Instance.Id;
}

ABox3DInstancedBodyActor* ABox3DInstancedBodyActor::GetOrCreateOverflowActor()
{
	if (ABox3DInstancedBodyActor* Existing = ActiveOverflowActor.Get())
	{
		const int32 MaxInstances = GetDefault<UBox3DProjectSettings>()->MaxInstancesPerActor;
		if (MaxInstances <= 0 || Existing->Bodies.Num() < MaxInstances)
		{
			return Existing;
		}
	}
	...
	ABox3DInstancedBodyActor* NewActor = World->SpawnActorDeferred<ABox3DInstancedBodyActor>(
		GetClass(), GetActorTransform(), GetOwner(), GetInstigator(),
	...
	NewActor->InstanceMesh = InstanceMesh;
	NewActor->BodyHalfExtent = BodyHalfExtent;
	NewActor->Density = Density;
	NewActor->LinearDamping = LinearDamping;
	NewActor->AngularDamping = AngularDamping;
	NewActor->bEnableSleep = bEnableSleep;
	NewActor->SleepThreshold = SleepThreshold;
	NewActor->ReplicationChunkSize = ReplicationChunkSize;
	NewActor->ReplicationChunkIntervalSeconds = ReplicationChunkIntervalSeconds;
	NewActor->bInterpolateReplicatedInstances = bInterpolateReplicatedInstances;
	NewActor->bMultiplayerCompatible = bMultiplayerCompatible;
	NewActor->ReplicatedInstanceInterpolationSpeed = ReplicatedInstanceInterpolationSpeed;
	NewActor->FinishSpawning(GetActorTransform());

	OverflowActors.Add(NewActor);
	ActiveOverflowActor = NewActor;
	return NewActor;
}
...
bool ABox3DInstancedBodyActor::AddImpulseToBody(int32 BodyId, const FVector& Impulse)
{
	...
	if (const FBodyRoute* Route = RoutedBodies.Find(BodyId))
	{
		if (ABox3DInstancedBodyActor* RoutedActor = Route->Actor.Get())
		{
			return RoutedActor->AddImpulseToBody(Route->LocalBodyId, Impulse);
		}
	}
...
void ABox3DInstancedBodyActor::UpdateSimulationInstances()
{
	...
	for (const FInstanceBody& Instance : Bodies)
	{
		...
		Transform = MakeSafeInstanceTransform(Transform);
...
void ABox3DInstancedBodyActor::OnRep_ReplicatedBodyChunk()
{
	...
	for (const FBox3DInstancedBodyState& State : PendingSnapshot)
	{
		...
		{
			FBox3DInstancedBodyState& SafeState = ClientTargetSnapshot.AddDefaulted_GetRef();
			SafeState = State;
			SafeState.WorldTransform = MakeSafeInstanceTransform(State.WorldTransform);
		}
	}
	if (!bInterpolateReplicatedInstances || ClientCurrentTransforms.Num() != ClientTargetSnapshot.Num())
	{
		...
		{
			ClientCurrentTransforms.Add(MakeSafeInstanceTransform(State.WorldTransform));
		}
	}
	for (const FTransform& Transform : ClientCurrentTransforms)
	{
		InstanceComponent->AddInstanceWorldSpace(MakeSafeInstanceTransform(Transform));
	}
	InstanceComponent->MarkRenderStateDirty();
...
{
	for (const TWeakObjectPtr<ABox3DInstancedBodyActor>& OverflowActor : OverflowActors)
	{
		if (ABox3DInstancedBodyActor* Actor = OverflowActor.Get())
		{
			Actor->Destroy();
		}
	}
	...
	Bodies.Reset();
	RoutedBodies.Reset();
	OverflowActors.Reset();
	ActiveOverflowActor.Reset();
