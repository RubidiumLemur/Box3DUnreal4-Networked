...
namespace
{
	...
	struct FRadialImpulseContext
	{
		...
		TSet<uint64> SeenBodies;
		int32 AppliedCount = 0;
	};

	uint64 MakeBodyKey(const b3BodyId Body)
	{
		return static_cast<uint64>(static_cast<uint32>(Body.index1)) |
			(static_cast<uint64>(Body.generation) << 32);
	}

	void ApplyRadialImpulseToBody(const b3BodyId Body, FRadialImpulseContext& Ctx)
	{
		if (B3_IS_NULL(Body) || !b3Body_IsValid(Body) || b3Body_GetType(Body) != b3_dynamicBody)
		{
			return;
		}

		const uint64 BodyKey = MakeBodyKey(Body);
		if (Ctx.SeenBodies.Contains(BodyKey))
		{
			return; // Compound/multi-shape bodies are affected only once.
		}
		Ctx.SeenBodies.Add(BodyKey);

		const FVector BodyLocation = Box3D::FromBox3DPosition(b3Body_GetWorldCenter(Body));
		const FVector Offset = BodyLocation - Ctx.Center;
		const float DistanceSquared = Offset.SizeSquared();
		const float RadiusSquared = FMath::Square(Ctx.Radius);
		...
		const float NormalizedDistance = FMath::Clamp(Distance / Ctx.Radius, 0.0f, 1.0f);
		const float Falloff = FMath::Pow(1.0f - NormalizedDistance,
			FMath::Max(0.001f, Ctx.FalloffExponent));
		const FVector Direction = DistanceSquared > SMALL_NUMBER ? Offset / Distance : FVector::UpVector;
		const FVector Impulse = (Direction * Ctx.Strength + FVector::UpVector * Ctx.UpwardStrength) * Falloff;
		b3Body_ApplyLinearImpulseToCenter(Body, Box3D::ToBox3DVector(Impulse), Ctx.bWakeBodies);
		++Ctx.AppliedCount;
	}

	bool RadialImpulseCallback(b3ShapeId ShapeId, void* Context)
	{
		FRadialImpulseContext& Ctx = *static_cast<FRadialImpulseContext*>(Context);
		ApplyRadialImpulseToBody(b3Shape_GetBody(ShapeId), Ctx);
		return true;
	}
...
int32 UBox3DSubsystem::ApplyRadialImpulse(const FVector& Center, float Radius, float Strength,
	float FalloffExponent, bool bWakeBodies, const FBox3DQueryFilter& Filter, float UpwardStrength)
{
	if (!bWorldValid || !bIsAuthority || Radius <= 0.0f || !FMath::IsFinite(Radius) ||
		!FMath::IsFinite(Strength) || !FMath::IsFinite(UpwardStrength) ||
		!FMath::IsFinite(FalloffExponent) || Center.ContainsNaN())
	{
		return 0;
	}

	FRadialImpulseContext Ctx;
	Ctx.Center = Center;
	Ctx.Radius = Radius;
	Ctx.Strength = Strength;
	Ctx.UpwardStrength = UpwardStrength;
	Ctx.FalloffExponent = FalloffExponent;
	Ctx.bWakeBodies = bWakeBodies;

	// The broadphase callback visits only shapes near the sphere. The callback deduplicates
	// compound/multi-shape bodies before doing the falloff calculation and applying the impulse.
	b3Vec3 Point;
	const b3ShapeProxy Proxy = MakeSphereProxy(Point, Radius);
	b3World_OverlapShape(WorldId, Box3D::ToBox3DPosition(Center), &Proxy, MakeQueryFilter(Filter),
		&RadialImpulseCallback, &Ctx);

	// Deliberately stay on the broadphase/spatial-cache path. This is the performant path for
	// high-count instanced actors and avoids an O(number of actor components) fallback scan.
	return Ctx.AppliedCount;
}

int32 UBox3DSubsystem::ApplyRadialImpulseSimple(const FVector& Center, float Radius, float Strength,
	float FalloffExponent, bool bWakeBodies, float UpwardStrength)
{
	return ApplyRadialImpulse(Center, Radius, Strength, FalloffExponent, bWakeBodies,
		FBox3DQueryFilter(), UpwardStrength);
}
